...
function PriceTicker({ prices }: { prices: PriceData[] }) {
  ...
  return (
    <div className="bg-slate-950/80 border-b border-slate-800/50 py-3 overflow-hidden relative">
      ...
      <div className="flex animate-ticker whitespace-nowrap">
        {tickerPrices.map((price, i) => (
          <div key={i} className="flex items-center gap-3 px-6 border-r border-slate-800/50 hover:bg-slate-800/30 transition-colors cursor-default">
            <span className="text-sm font-medium text-slate-300">{price.symbol}</span>
            <span className="text-sm font-mono text-white">{price.bid.toFixed(price.symbol.includes("JPY") || price.symbol.includes("XAU") || price.symbol.includes("BTC") ? 2 : 4)}</span>
            <span className={`text-xs font-medium ${price.change >= 0 ? "text-green-400" : "text-red-400"}`}>
        ...
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          ...
          >
            <div className="inline-flex items-center gap-2 bg-green-500/10 border border-green-500/30 rounded-full px-4 py-2 mb-6">
              ...
              <span className="text-sm text-green-400 font-medium"> {t("marketsLive")}</span>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-6 text-sm">
              {[
                ...
                <div key={i} className="flex items-center gap-2 text-slate-400">
                  <div className="w-6 h-6 rounded-full bg-green-500/20 flex items-center justify-center text-green-400 text-xs">{item.icon}</div>
                  <span>{item.text}</span>
                </div>
              ))}
            </div>
          ...
          <div className="hidden lg:block relative">
            ...
            >
              <div className="flex items-center justify-between mb-6">
                <span className="text-sm text-slate-400">{t("liveMarketPrices")}</span>
                <div className="flex items-center gap-2">
                  <span className="w-2 h-2 bg-green-400 rounded-full animate-pulse" />
                  <span className="text-xs text-green-400">{t("realtime")}</span>
                </div>
              </div>

              <div className="space-y-1">
                {prices.slice(0, 8).map((price, i) => (
                  <div key={i} className="flex items-center justify-between py-3 border-b border-slate-700/30 last:border-0 hover:bg-slate-800/30 -mx-4 px-4 rounded transition-colors cursor-pointer">
                    <div className="flex items-center gap-3">
                      <div className={`w-2 h-2 rounded-full ${price.change >= 0 ? "bg-green-400" : "bg-red-400"}`} />
              ...
              <Link href="/dashboard" className="block mt-4 text-center text-sm text-cyan-400 hover:underline py-2 border-t border-slate-700/30">
                {t("viewAllMarkets")} →
              </Link>
            </motion.div>
            ...
            >
              <div className="text-xs text-slate-400 mb-2">{t("todaysProfit")}</div>
              <div className="text-3xl font-bold text-green-400">+$12,450.00</div>
              <div className="text-xs text-slate-500 mt-1">+24.5% return</div>
            </motion.div>
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="text-xs text-slate-400">{t("brokerAward")}</div>
                </div>
              </div>
            </motion.div>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
          {statsData.map((stat, i) => (
            <div key={i} className="text-center group">
              ...
              <div className="text-sm text-slate-400 group-hover:text-cyan-400 transition-colors">{stat.label}</div>
            </div>
          ))}
        </div>
      </div>
...
function ProductsSection() {
  ...
  const productsList = [
    { nameKey: "forex", icon: "💱", descKey: "forexDesc", spread: "0.0", link: "/products/forex", color: "from-blue-500 to-cyan-500" },
    ...
    <section id="products" className="py-24">
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <div className="text-center mb-16">
          <span className="text-cyan-400 text-sm font-semibold tracking-wider uppercase">{t("tradingProductsSection")}</span>
          <h2 className="text-4xl lg:text-5xl font-bold mt-4 mb-4">{t("allMarketsTitle")}</h2>
          <p className="text-slate-400 max-w-2xl mx-auto text-lg">{t("allMarketsDesc")}</p>
        </div>
          ...
          {productsList.map((product, i) => (
            <Link key={i} href={product.link} className="glass-card rounded-2xl p-6 hover-lift group cursor-pointer relative overflow-hidden">
              ...
              <div className="relative">
                ...
                <h3 className="text-xl font-bold mb-2 group-hover:text-cyan-400 transition-colors">{t(product.nameKey as any)}</h3>
                <p className="text-slate-400 text-sm mb-4">{t(product.descKey as any)}</p>
                <div className="flex items-center justify-between">
                  <span className="text-cyan-400 text-sm font-semibold">{t("fromPipsValue", { spread: product.spread })}</span>
                  <span className="text-slate-500 text-sm group-hover:text-cyan-400 transition-colors flex items-center gap-1">
...
function PlatformsSection() {
  ...
  return (
    <section id="platforms" className="py-24 bg-gradient-to-b from-slate-900/50 to-transparent">
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <div className="grid lg:grid-cols-2 gap-16 items-center">
          <div>
            <span className="text-cyan-400 text-sm font-semibold tracking-wider uppercase">{t("tradingPlatforms")}</span>
            <h2 className="text-4xl lg:text-5xl font-bold mt-4 mb-6">{t("metaTrader5")} - {t("industryStandard")}</h2>
            <p className="text-slate-400 text-lg mb-8">{t("metaTraderDesc")}</p>

            <div className="grid grid-cols-2 gap-4 mb-8">
              {platformFeatures.map((feature, i) => (
                <div key={i} className="glass rounded-xl p-4 hover:bg-slate-800/50 transition-colors">
                  <div className="text-3xl mb-3">{feature.icon}</div>
                  <div className="font-semibold mb-1">{t(feature.titleKey as any)}</div>
                  <div className="text-sm text-slate-400">{t(feature.descKey as any)}</div>
                </div>
              ))}
            </div>
          ...
          <div className="relative">
            <div className="glass-card rounded-2xl p-8 text-center relative overflow-hidden">
              ...
              <div className="relative">
                ...
                <p className="text-slate-400 mb-8">{t("tradeAnywhereDesc")}</p>
                ...
                <div className="flex justify-center gap-4 flex-wrap">
                  {[
                    ...
                    <a key={i} href={app.href} className="flex items-center gap-2 bg-slate-800/80 rounded-xl px-4 py-3 hover:bg-slate-700/80 transition-colors">
                      <span className="text-2xl">{app.icon}</span>
                      <div className="text-left">
                        <div className="text-xs text-slate-400">{t(app.labelKey as any)}</div>
            ...
            <div className="absolute -bottom-4 -right-4 glass-card rounded-xl p-4 animate-bounce" style={{ animationDuration: "3s" }}>
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="text-xs text-slate-400">{t("liveAccount")}</div>
                </div>
              </div>
            </div>
          </div>
...
function AccountTypesSection({ openRegister }: { openRegister: () => void }) {
  ...
  return (
    <section id="accounts" className="py-24">
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <div className="text-center mb-16">
          <span className="text-cyan-400 text-sm font-semibold tracking-wider uppercase">{tf("accountTypesSection")}</span>
          <h2 className="text-4xl lg:text-5xl font-bold mt-4 mb-4">{t("chooseAccount")}</h2>
          <p className="text-slate-400 max-w-2xl mx-auto text-lg">{t("transparentPricing")}</p>
        </div>

        <div className="grid md:grid-cols-3 gap-8">
          {accountTypes.map((account, i) => (
            <div key={i} className={`glass-card rounded-2xl p-8 hover-lift relative ${i === 1 ? "border-cyan-500/50 ring-2 ring-cyan-500/20 scale-105" : ""}`}>
              {i === 1 && (
                <div className="absolute -top-4 left-1/2 -translate-x-1/2 bg-gradient-to-r from-cyan-500 to-blue-500 text-slate-900 text-xs font-bold px-6 py-2 rounded-full shadow-lg shadow-cyan-500/30">
                  ⭐ {t("mostPopular")}
                </div>
              )}
              <div className="text-center mb-6">
                ...
                <div className="text-sm text-slate-400">{t("averageSpread")}</div>
              </div>

              <div className="space-y-4 mb-8">
                <div className="flex justify-between py-3 border-b border-slate-700/50">
                  <span className="text-slate-400">{t("commission")}</span>
                  <span className="font-semibold">{account.commission}</span>
                </div>
                <div className="flex justify-between py-3 border-b border-slate-700/50">
                  <span className="text-slate-400">{t("minDeposit")}</span>
                  <span className="font-semibold">{account.minDeposit}</span>
                </div>
                <div className="flex justify-between py-3 border-b border-slate-700/50">
                  <span className="text-slate-400">{t("maxLeverage")}</span>
...
function FeaturesSection() {
  ...
  return (
    <section className="py-24 bg-gradient-to-b from-slate-900/50 to-transparent">
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <div className="text-center mb-16">
          <span className="text-cyan-400 text-sm font-semibold tracking-wider uppercase">{t("whyDolphinMarkets")}</span>
          <h2 className="text-4xl lg:text-5xl font-bold mt-4 mb-4">{t("tradersChoice")}</h2>
          <p className="text-slate-400 max-w-2xl mx-auto text-lg">{t("tradersTrust")}</p>
        </div>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {features.map((feature, i) => (
            <div key={i} className="glass-card rounded-xl p-6 hover-lift group">
              ...
              <h3 className="text-lg font-semibold mb-2 group-hover:text-cyan-400 transition-colors">{t(feature.titleKey as any)}</h3>
              <p className="text-sm text-slate-400">{t(feature.descKey as any)}</p>
            </div>
          ))}
        </div>
      </div>
...
function CTASection({ openRegister }: { openRegister: () => void }) {
  ...
  return (
    <section className="py-24 relative overflow-hidden">
      <div className="absolute inset-0 bg-gradient-to-r from-cyan-900/30 via-transparent to-purple-900/30" />
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 text-center relative z-10">
        ...
        <p className="text-xl text-slate-400 mb-10">{t("openMinutes")}</p>
        <div className="flex flex-wrap justify-center gap-4">
          <button onClick={openRegister} className="btn-primary text-lg px-10 py-5 shadow-lg shadow-cyan-500/30">
            {t("createLiveAccount")}
          </button>
...
function TestimonialsSection() {
  ...
  return (
    <section className="py-24 bg-gradient-to-b from-transparent to-slate-900/50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <div className="text-center mb-16">
          <span className="text-cyan-400 text-sm font-semibold tracking-wider uppercase">{t("testimonials")}</span>
          <h2 className="text-4xl lg:text-5xl font-bold mt-4 mb-4">{t("trustedByTraders")}</h2>
          <p className="text-slate-400 max-w-2xl mx-auto text-lg">{t("whatClientsSay")}</p>
        </div>
          ...
          {testimonials.map((testimonial, i) => (
            <div key={i} className="glass-card rounded-xl p-6">
              <div className="flex items-center gap-3 mb-4">
                <div className="w-12 h-12 rounded-full bg-gradient-to-br from-cyan-500 to-blue-500 flex items-center justify-center text-lg font-bold text-slate-900">
                  {testimonial.name[0]}
                </div>
                <div>
                  <div className="font-semibold">{testimonial.name}</div>
                  <div className="text-sm text-slate-400">{t(testimonial.roleKey as any)}</div>
                </div>
              </div>
              <p className="text-slate-300 text-sm mb-4">&quot;{t(testimonial.textKey as any)}&quot;</p>
        ...
        <div className="mt-16 glass-card rounded-xl p-8">
          <div className="text-center mb-8">
            ...
            <p className="text-sm text-slate-400">{t("securityMeasures")}</p>
          </div>
    ...
    <section className="py-16 border-t border-slate-800/50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <div className="text-center mb-12">
          <span className="text-cyan-400 text-sm font-semibold tracking-wider uppercase">{t("securityRegulation")}</span>
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {regulations.map((item, i) => (
            <div key={i} className="text-center">
              ...
              <p className="text-sm text-slate-400">{t(item.descKey as any)}</p>
            </div>
          ))}
        </div>
      </div>
...
function AnnouncementBar() {
  ...
  return (
    <div className="relative z-40 bg-gradient-to-r from-cyan-900/40 via-cyan-800/30 to-cyan-900/40 border-b border-cyan-700/30 py-2.5">
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <div className="flex items-center gap-3 text-sm">
          <span className="text-cyan-400 animate-pulse">📢</span>
          <span className="text-slate-300">{t("marketUpdate")}: {t("usSessionOpen")}</span>
          <span className="ml-auto">
            <Link href="/trading-tools" className="text-cyan-400 hover:underline text-xs">{t("viewCalendar")} →</Link>
          </span>
        </div>
      </div>
    </div>
